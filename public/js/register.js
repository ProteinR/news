...
var _register = new Vue({
  ...
  methods: {
    register: function register() {
      ...
      _axios_global__WEBPACK_IMPORTED_MODULE_0__["AXIOS"].post('/api/auth/account', {
        "name": this.name,
        "email": this.email,
    ...
    login: function login() {
      ...
      _axios_global__WEBPACK_IMPORTED_MODULE_0__["AXIOS"].post('/api/auth/token', {
        "email": this.email,
        "password": this.password
      }).then(function (response) {
        _register.registerResponse = response.data;
        ...
        if (response.data.message == 'Authorization Successful!') {
          ...
          swal({
            title: "Успех!",
            text: "Вы вошли как " + response.data.currentUser.name + "! Вы будете перенаправлены на" + " главную страницу через" + " несколько секунд...",
            icon: "success",
            button: "Aww yiss!",
            timer: 3000
          });
